...
function apiInstanceFromPath(cfgPath, api){
  ...
  log.trace('Loading config start ====================================');
  log.trace('Config:%s', JSON.stringify(cfg));
  log.trace('Loading config end   ====================================');
...
function apiInstanceFromJson(cfg, api){
  ...
  keys.forEach(function(v,i){
    this[v] = function(){
      ...
      //if opt overwrite
			if(cfg.API_CFG['REQ_OPTS'])
		    opts = _.extend(opts, cfg.API_CFG['REQ_OPTS']);
      ...
      //if header exist
      if(api[v].headers)  {
        log.trace('Set header using single api config...%s', JSON.stringify(api[v].headers));
        if(!API_HEADER) {
        ...
        log.trace('Set header using global config...%s', JSON.stringify(api[v].headers));
      ...
      log.trace('notvalidate for the api is %s', api[v].notvalidate);
  ...
  function convertUrl(base_url, args){
    ...
    for( var i = 1 ; i < urltoken.length ; i++ ){
      var key = urltoken[i];
      if( key.indexOf(':') == 0 ){
        log.trace('values:%s, type:%s', values[0], typeof(values[0]));
  ...
  function validator(inargs, args){
    ...
    for(var i = 0 ; i < args.length ; i++) {
      ...
      log.trace('Type of argument %s is [%s], defined type is [%s]',inargs[i], typeof(inargs[i]), args[i].type);
